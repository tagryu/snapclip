      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative p-6 md:p-8 rounded-2xl border transition-all duration-300 card-glow ${
                plan.highlight
                  ? "bg-card border-accent-purple/50 shadow-lg shadow-purple-500/10 md:scale-105"
                  : "bg-card border-card-border"
              }`}
            >
              {plan.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 text-xs font-medium rounded-full gradient-bg text-white">
                  🔥 가장 인기
                </div>
              )}
              ...
              <p className="text-sm text-accent-purple font-medium mb-8">영상 {plan.videos}</p>
              <ul className="space-y-3 mb-8">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-center gap-2.5 text-sm text-muted">
                    <svg className="w-4.5 h-4.5 text-accent-purple shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="mt-20 max-w-2xl mx-auto">
          ...
          {[
            ...
            <details key={faq.q} className="group border-b border-card-border">
              <summary className="flex items-center justify-between py-4 cursor-pointer text-sm font-medium hover:text-accent-purple transition-colors">
                {faq.q}
