...
exports.ZP = {
    ...
    slowZoomIn: (dur, fps = 30) => `zoompan=z='min(zoom+0.0008,1.12)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    ...
    fastZoomOut: (dur, fps = 30) => `zoompan=z='if(eq(on,1),1.6,max(zoom-0.006,1))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Ken Burns - slow diagonal pan with zoom */
    kenBurns: (dur, fps = 30) => `zoompan=z='min(zoom+0.001,1.2)':x='if(eq(on,1),0,min(x+1,iw-iw/zoom))':y='if(eq(on,1),0,min(y+0.5,ih-ih/zoom))':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Static with subtle pulse */
    pulse: (dur, fps = 30) => `zoompan=z='1.02+0.02*sin(on*2*PI/${fps}/2)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Snap zoom in — very fast zoom for reels/tiktok impact */
    snapZoomIn: (dur, fps = 30) => `zoompan=z='min(1+0.5*pow(on/${dur * fps},0.3),1.5)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Zoom in then out (breathe) */
    breathe: (dur, fps = 30) => `zoompan=z='1.1+0.15*sin(on*PI/${dur * fps})':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Quick cut static — minimal motion, just holds */
    quickStatic: (dur, fps = 30) => `zoompan=z='1.05':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Camera shake + zoom in — fast zoom with 3-5px random offset for energy */
    shakeZoomIn: (dur, fps = 30) => `zoompan=z='min(zoom+0.004,1.4)':x='iw/2-(iw/zoom/2)+random(1)*5-2':y='ih/2-(ih/zoom/2)+random(2)*5-2':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Spiral zoom — zoom with circular x/y movement */
    spiralZoom: (dur, fps = 30) => `zoompan=z='min(zoom+0.002,1.25)':x='iw/2-(iw/zoom/2)+15*sin(on*2*PI/${dur * fps})':y='ih/2-(ih/zoom/2)+15*cos(on*2*PI/${dur * fps})':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Snap zoom punch — ultra fast zoom in then snap back (punch effect) */
    snapZoomPunch: (dur, fps = 30) => {
        const d = dur * fps;
    ...
    panDiagonal: (dur, fps = 30) => `zoompan=z='min(zoom+0.001,1.15)':x='if(eq(on,1),0,min(x+1.5,iw-iw/zoom))':y='if(eq(on,1),0,min(y+1,ih-ih/zoom))':d=${dur * fps}:s=1080x1080:fps=${fps}`,
    /** Slow drift — very subtle random drift movement (luxury feel) */
    slowDrift: (dur, fps = 30) => `zoompan=z='1.08+0.02*sin(on*PI/${dur * fps})':x='iw/2-(iw/zoom/2)+8*sin(on*PI/${dur * fps}/2)':y='ih/2-(ih/zoom/2)+5*cos(on*PI/${dur * fps}/3)':d=${dur * fps}:s=1080x1080:fps=${fps}`,
};
