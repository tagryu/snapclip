 ...
 */
async function createTextOverlay(overlay, width, height, outputPath) {
    const textElements = [];
    for (const line of overlay.lines) {
        ...
        textElements.push(`<text x="${width / 2 + shadowOff}" y="${y + shadowOff}" ` +
        ...
        // Outline stroke
        textElements.push(`<text x="${width / 2}" y="${y}" ` +
            `font-family="Pretendard, Apple SD Gothic Neo, sans-serif" ` +
            `font-size="${line.fontSize}" font-weight="bold" ` +
 ...
 */
async function generateBGM(outputPath, durationSec) {
    ...
    logger_1.logger.info(`BGM generated: ${outputPath}`);
    return outputPath;
}
/**
 ...
 */
function buildSegmentTexts(productName, productPrice, aiCopy, features, fontColor, segmentCount = 5) {
 ...
 */
async function generateSFX(outputDir, segments, templateName) {
    try {
        const sfxPath = path_1.default.join(outputDir, 'sfx_transitions.aac');
        ...
        for (let i = 0; i < transitionTimes.length; i++) {
            ...
            const dur = 0.15;
            whooshParts.push(`aevalsrc='0.3*sin(2*PI*(${startFreq}-${startFreq - 200}*t/${dur})*t)*exp(-t/${dur}*3)':s=44100:d=${dur}[w${i}]`);
            // Pad with silence to position at correct time
            whooshParts.push(`[w${i}]adelay=${Math.round(t * 1000)}|${Math.round(t * 1000)}[wd${i}]`);
            mixLabels.push(`[wd${i}]`);
        }
        // Mix all whooshes together, pad to total duration
        const mixInput = mixLabels.join('');
        whooshParts.push(`${mixInput}amix=inputs=${transitionTimes.length}:duration=longest,` +
            `apad=whole_dur=${totalDuration},volume=-20dB`);
        const filter = whooshParts.join(';\n');
        const filterFile = path_1.default.join(outputDir, 'sfx_filter.txt');
        await promises_1.default.writeFile(filterFile, filter);
        const cmd = `ffmpeg -y -filter_complex_script "${filterFile}" -t ${totalDuration} -c:a aac -b:a 64k "${sfxPath}"`;
        await execAsync(cmd, { timeout: 30000 });
        logger_1.logger.info(`SFX generated: ${sfxPath} (${transitionTimes.length} transitions)`);
        return sfxPath;
    }
    catch (err) {
        logger_1.logger.warn(`SFX generation failed (non-fatal): ${err.message}`);
        return undefined;
    }
}
/**
 ...
 */
function getTextAnimationFilters(templateName, segmentDuration) {
    switch (templateName) {
        case 'trendy':
...
async function composeVideo(opts) {
    const { images, template: templateName, aspect, productName, productPrice, aiCopy, productFeatures = [], bgmPath, narrationPath, outputDir, bpm, } = opts;
    ...
    if (bpm && bpm > 0) {
        ...
        logger_1.logger.info(`BPM sync: ${bpm} BPM, beat interval=${beatInterval.toFixed(3)}s`);
    }
    const outputPath = path_1.default.join(outputDir, `output_${aspect.label.replace(':', 'x')}_${Date.now()}.mp4`);
    const { width, height } = aspect;
    const totalDuration = segments.reduce((s, seg) => s + seg.duration, 0);
    logger_1.logger.info(`Composing video: ${templateName}, ${aspect.label}, ${images.length} images, ${totalDuration}s`);
    // Scale all source images
    const scaledImages = [];
    const uniqueIndices = [...new Set(segments.map(s => s.imageIndex))];
    const scaledMap = new Map();
    for (const idx of uniqueIndices) {
        const scaledPath = path_1.default.join(outputDir, `scaled_${idx}.png`);
        await (0, sharp_1.default)(images[idx])
    ...
    const overlayPaths = [];
    for (let i = 0; i < segments.length; i++) {
        const overlayPath = path_1.default.join(outputDir, `overlay_seg${i}.png`);
    ...
    if (!actualBgmPath) {
        actualBgmPath = path_1.default.join(outputDir, 'bgm_generated.aac');
        try {
            await generateBGM(actualBgmPath, totalDuration);
        }
        catch (err) {
            logger_1.logger.warn(`BGM generation failed: ${err.message}`);
    ...
    for (let i = 0; i < numSeg; i++) {
        ...
        if (isReels) {
            ...
            filters.push(`[${overlayIdx}:v]format=yuva420p,fade=in:st=0:d=0.3:alpha=1[otxt${i}]`);
            filters.push(`[v${i}][otxt${i}]overlay=0:'if(lt(t,0.3),30*(1-t/0.3),0)':shortest=1[vt${i}]`);
        }
        else {
    ...
    const filterFile = path_1.default.join(outputDir, 'filter_complex.txt');
    await promises_1.default.writeFile(filterFile, allFilters.join(';\n'));
    const outputArgs = [
        '-map', '[final]',
        ...(hasAudio ? ['-map', '[aout]'] : []),
        '-c:v', 'libx264',
    ...
    logger_1.logger.info(`FFmpeg args count: ${allArgs.length}`);
    logger_1.logger.debug?.(`FFmpeg full args: ${allArgs.join(' ')}`);
    return new Promise((resolve, reject) => {
        const proc = (0, child_process_1.spawn)('ffmpeg', allArgs, { stdio: ['pipe', 'pipe', 'pipe'] });
        let stderr = '';
        ...
        proc.on('close', (code) => {
            if (code === 0) {
                ...
                logger_1.logger.error(`FFmpeg stderr: ${stderr.slice(-1000)}`);
