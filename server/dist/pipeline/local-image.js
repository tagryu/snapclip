...
exports.generateRealisticBackground_local = generateRealisticBackground_local;
exports.generateMultiAngle_local = generateMultiAngle_local;
...
// ─── ComfyUI API Helpers ───
async function isComfyAvailable(config) {
    try {
...
async function uploadImageToComfy(config, imagePath, filename) {
...
async function queueComfyWorkflow(config, workflow) {
...
async function waitForResult(config, promptId, timeoutMs = 120000) {
            ...
            if (entry?.outputs) {
                ...
                for (const nodeId of Object.keys(entry.outputs)) {
                    ...
                    if (output.images && output.images.length > 0) {
                        ...
                        const imgRes = await fetch(`${config.host}/view?filename=${img.filename}&subfolder=${img.subfolder || ''}&type=${img.type || 'output'}`);
                        if (imgRes.ok) {
...
function buildBackgroundReplaceWorkflow(inputFilename, scene, aspectRatio) {
...
function buildMultiAngleWorkflow(inputFilename, angle) {
...
async function fallbackSimulatedAngle(imagePath, angle, outputDir) {
...
// ─── Exported Functions ───
async function generateRealisticBackground_local(productImagePath, scene, aspectRatio, outputDir) {
    await promises_1.default.mkdir(outputDir, { recursive: true });
    const config = getComfyConfig();
    if (!config) {
        logger_1.logger.info('No COMFYUI_URL configured, skipping local image generation');
        return productImagePath;
    }
    if (!(await isComfyAvailable(config))) {
        logger_1.logger.warn('ComfyUI server not reachable, skipping local background generation');
        return productImagePath;
    }
    try {
        ...
        const outputPath = path_1.default.join(outputDir, `local_bg_${Date.now()}.png`);
        await promises_1.default.writeFile(outputPath, resultBuffer);
        logger_1.logger.info(`Local realistic background generated: ${outputPath}`);
        return outputPath;
    }
    catch (err) {
        logger_1.logger.warn(`Local background generation failed: ${err.message}`);
        return productImagePath;
    }
}
async function generateMultiAngle_local(productImagePath, angles, outputDir) {
    await promises_1.default.mkdir(outputDir, { recursive: true });
    const config = getComfyConfig();
    if (!config || !(await isComfyAvailable(config))) {
        logger_1.logger.info('ComfyUI not available, using Sharp fallback for multi-angle');
    ...
    try {
        ...
        for (const angle of angles) {
            try {
                ...
                const promptId = await queueComfyWorkflow(config, workflow);
                const resultBuffer = await waitForResult(config, promptId);
                const outputPath = path_1.default.join(outputDir, `local_angle_${angle.replace(/\s+/g, '_')}_${Date.now()}.png`);
                await promises_1.default.writeFile(outputPath, resultBuffer);
                results.push(outputPath);
                logger_1.logger.info(`Local multi-angle ${angle} generated: ${outputPath}`);
            }
            catch (err) {
                logger_1.logger.warn(`Local multi-angle ${angle} failed: ${err.message}`);
        ...
        logger_1.logger.warn(`Local multi-angle generation failed entirely: ${err.message}`);
        return fallbackMultiAngle_local(productImagePath, angles, outputDir);
    }
}
async function fallbackMultiAngle_local(imagePath, angles, outputDir) {
    const results = [];
    for (const angle of angles) {
